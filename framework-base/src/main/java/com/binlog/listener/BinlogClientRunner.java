...
public class BinlogClientRunner implements CommandLineRunner {
    ...
    @Value("${binlog.database.tables}")
    private String database_table;

    @Value("${binlog.database.host}")
    private String host;

    @Value("${binlog.database.port}")
    private int port;

    @Value("${binlog.database.username}")
    private String user;

    @Value("${binlog.database.password}")
    private String password;

    @Value("${binlog.database.serviceid}")
                ...
                if (data instanceof UpdateRowsEventData) {
                    ...
                    if (tableName != null && databaseList.contains(tableName)) {
                        ...
                        for (Map.Entry<Serializable[], Serializable[]> row : updateRowsEventData.getRows()) {
                            ...
                            System.out.println(eventKey+":"+row.toString());
                        }
                    }
                }
                // insert数据
                    ...
                    if (tableName != null && databaseList.contains(tableName)) {
                        ...
                        for (Serializable[] row : writeRowsEventData.getRows()) {
                            ...
                            //kafkaSender.send(topic, msg);
                            System.out.println(eventKey+":"+row);
                        }
                    }
                }
                // delete数据
